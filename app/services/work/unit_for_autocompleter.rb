class Work::UnitForAutocompleter
  def self.find(query)
    Work::Unit.find_by_sql([%Q{SELECT id, (repeat( '> ', ancestry_depth-1) || name) as name FROM (

 SELECT id, name, type, ancestry_depth,
      (
        ARRAY_TO_STRING(
	        ARRAY_APPEND(
        ...
        WITH work_units_matched AS (
          SELECT * FROM work_units
                WHERE work_units.name ~* ?
                        ...
                        work_units.wuid ~* ?
                      AND
                      work_units.ancestry_depth IN (1,2,3)
        )
